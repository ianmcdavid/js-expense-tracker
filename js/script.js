...
const showMinMax = function (expenses) {
  ...
  const minest = document.createElement("li");
  minest.innerText = `Min: $ ${min}`;
  const maxest = document.createElement("li");
  maxest.innerText = `Max: $ ${max}`;
  results.append(minest);
  results.append(maxest);
